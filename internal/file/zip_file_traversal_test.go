...
func TestUnzipToDir(t *testing.T) {
	...
	unzipDestinationDir, err := ioutil.TempDir("", "gosbom-ziputil-contents-TEST-")
...
func prependZipSourceFixtureWithString(tb testing.TB, value string) func(tb testing.TB) string {
	...
	return func(t testing.TB) string {
		...
		// create a temp file
		tmpFile, err := ioutil.TempFile("", "gosbom-ziputil-prependZipSourceFixtureWithString-")
		if err != nil {
...
func prepZipSourceFixture(t testing.TB) string {
	t.Helper()
	archivePrefix, err := ioutil.TempFile("", "gosbom-ziputil-prepZipSourceFixture-")
	if err != nil {
		t.Fatalf("unable to create tempfile: %+v", err)
	}
